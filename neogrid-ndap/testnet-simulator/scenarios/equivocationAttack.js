...
async function singleRun({ nodeCount }) {
  ...
  const protocolTrace = [{ round: 0, event: { type: 'PRECOMMIT', height: 0 }, stateBefore: { height: 0, view: 0, validatorSet: vs.getAll(), smtRoot: null, mmrRoot: null, finalizedStateRoot: null }, stateAfter: { height: 0, view: 0, validatorSet: vs.getAll(), smtRoot: 'honest-root', mmrRoot: 'honest-mmr', finalizedStateRoot: 'honest-root' }, signatures: [] }];
  return { scenario: 'equivocationAttack', pass, detail: pass ? 'equivocation contained' : checker.summary(), convergenceMs: Date.now() - start, failureClassification: pass ? null : 'safety', worstCaseTrace: all.flatMap((n) => n.getLog()), protocolTrace };
}

function aggregate(scenario, runs, summary) {
  return { scenario, pass: runs.every((r) => r.pass), successRate: Math.round((runs.filter((r) => r.pass).length / runs.length) * 1000) / 10, failureDistribution: failureCounts(runs), convergenceHistogram: histogramFromRuns(runs.map((r) => r.convergenceMs || 0)), worstCasePathTrace: runs.reduce((a, b) => (a.convergenceMs || 0) > (b.convergenceMs || 0) ? a : b, runs[0]).worstCaseTrace || [], protocolTrace: runs.reduce((a, b) => (a.convergenceMs || 0) > (b.convergenceMs || 0) ? a : b, runs[0]).protocolTrace || [], quorumStabilityScore: summary.convergenceStability, finalitySuccessRate: summary.finalitySuccessRate, failureClassification: summary.failureClassification, resilienceScore: summary.resilienceScore, convergenceStability: summary.convergenceStability, adversarialResistance: summary.adversarialResistance, safety: summary.safety, runs };
}
